...
function Section({ n, title, children }: { n: number; title: string; children: React.ReactNode }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="bg-navy-500 px-5 py-3 flex items-center gap-3">
...
const inputClass = 'w-full px-3 py-2.5 text-sm border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-400 bg-white'
...
function YesNo({ name, value, onChange }: { name: string; value: string; onChange: (v: string) => void }) {
  return (
    <div className="flex gap-3">
      {['sim', 'nao'].map((opt) => (
        <label key={opt} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-medium cursor-pointer transition-colors ${value === opt ? 'border-sky-400 bg-sky-50 text-sky-700' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
    ...
    <form action={formAction} className="space-y-5">
      ...
      <Section n={1} title="Contato">
        <Field label="Endereço de e-mail do responsável" required>
          <input type="email" name="email_responsavel" required className={inputClass} placeholder="email@exemplo.com" />
        </Field>
      </Section>
      ...
      <Section n={2} title="Termos e Condições">
        <p className="text-sm text-gray-600 leading-relaxed">
          Eu, responsável legal pelo(a) aluno(a) inscrito(a) na Escolinha de Triathlon São José, declaro que li, compreendi e concordo com os termos abaixo:
        </p>
        <ol className="space-y-3">
          {TERMOS.map((t, i) => (
            <li key={i} className="flex gap-3 text-sm text-gray-600">
              <span className="flex-shrink-0 w-5 h-5 bg-sky-100 text-sky-600 rounded-full text-[11px] font-bold flex items-center justify-center mt-0.5">{i + 1}</span>
              <span className="leading-relaxed">{t}</span>
            </li>
          ))}
        </ol>
        <label className={`flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-colors ${aceite ? 'border-emerald-400 bg-emerald-50' : 'border-gray-200 hover:bg-gray-50'}`}>
          <input
            type="checkbox"
            checked={aceite}
      ...
      {/* Dados do aluno */}
      <Section n={4} title="Dados do Atleta (Candidato à Vaga)">
        <Field label="Nome completo do aluno" required>
          <input type="text" name="nome" required className={inputClass} placeholder="Nome completo" />
        </Field>
        ...
        <Field label="CPF do aluno" hint="Somente números ou com pontuação">
          <input type="text" name="cpf" className={inputClass} placeholder="000.000.000-00" />
        </Field>
        <Field label="Endereço completo de residência">
          <textarea name="endereco_completo" rows={2} className={inputClass + ' resize-none'} placeholder="Rua, número, bairro, cidade, CEP" />
        </Field>
      </Section>

      {/* Escola */}
      <Section n={5} title="Dados Escolares">
        <Field label="Nome e endereço completo da escola onde o aluno está matriculado">
          <textarea name="escola_nome_endereco" rows={2} className={inputClass + ' resize-none'} placeholder="Nome da escola e endereço" />
        </Field>
        <Field label="Série/ano escolar em que o aluno está matriculado">
          <input type="text" name="serie_escolar" className={inputClass} placeholder="Ex: 5º ano do Ensino Fundamental" />
        </Field>
      </Section>
      ...
      <Section n={6} title="Informações de Saúde">
        ...
        {condicaoMedica === 'sim' && (
          <Field label="Descreva detalhadamente a condição médica ou restrição física" required>
            <textarea name="condicao_medica_descricao" rows={3} required className={inputClass + ' resize-none'} placeholder="Descreva aqui..." />
          </Field>
        )}
        ...
        {tratamento === 'sim' && (
          <Field label="Descreva o tratamento médico ou uso de medicação contínua" required>
            <textarea name="tratamento_medico_descricao" rows={3} required className={inputClass + ' resize-none'} placeholder="Descreva aqui..." />
          </Field>
        )}
        ...
        {alergia === 'sim' && (
          <Field label="Descreva a alergia" required>
            <input type="text" name="alergia_descricao" required className={inputClass} placeholder="Ex: alergia a medicamentos, alimentos..." />
          </Field>
        )}

        <Field label="O aluno possui autorização médica para a prática esportiva?" hint="Orientamos fortemente que todos os alunos realizem o exame pré-participação com médico do esporte ou cardiologista.">
      ...
      <Section n={7} title="Histórico Esportivo">
        ...
        <Field label="Como você ficou sabendo do Projeto Escolinha de Triathlon São José?">
          <textarea name="como_soube" rows={2} className={inputClass + ' resize-none'} placeholder="Ex: redes sociais, amigos, escola..." />
        </Field>
      </Section>
      ...
      <Section n={8} title="Dados do Responsável Legal">
        <Field label="Nome completo do responsável legal" required>
          <input type="text" name="responsavel_nome" required className={inputClass} placeholder="Nome completo" />
        </Field>
        <Field label="Telefone / WhatsApp para contato" required>
          <input type="tel" name="responsavel_telefone" required className={inputClass} placeholder="(48) 99999-9999" />
        </Field>
        <Field label="E-mail do responsável legal">
          <input type="email" name="responsavel_email" className={inputClass} placeholder="email@exemplo.com" />
        </Field>
      </Section>
      ...
      <button
        ...
        className="w-full bg-sky-400 hover:bg-sky-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold text-base py-4 rounded-2xl transition-colors shadow-lg"
      >
